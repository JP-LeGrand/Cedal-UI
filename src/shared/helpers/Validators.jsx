...
export const validateBankAccountNumber = (value) => {
    if (!isNumberNumber(value)) { return 'Only numeric characters' }
...
export const initialSurnameValidator = (initialMin, initialMax, surnameMin, surnameMax) => (input) => {
    if (!input) { return 'Value must not be blank' }
    const split = splitOnSpace(input)
    const initials = split[0]
    const surname = split.slice(1).join(' ')
    if (initialMin && initials.length < initialMin) { return 'Too few initials' }
    if (initialMax && initials.length > initialMax) { return 'Too many initials' }
    if (surnameMin && surname.length < surnameMin) { return 'Surname too short' }
    if (surnameMax && surname.length > surnameMax) { return 'Surname too long' }
}

export const isValidCompanyNumber = (number) => {
    if (!number || number.length !== 14) { return false }

    if (number.charAt(4) !== '/' || number.charAt(11) !== '/') { return false }
...
export const isValidStateOwnedCompany = (number) => {
    return isValidLastTwoDigits(number, '06') || isValidLastTwoDigits(number, '30')
}
...
export const validateIdNumber = (idNumber, dob) => {
    ...
    if (newDob) {
        const strippedDOB =
            newDob.getFullYear() + '' +
            (newDob.getMonth() + 1).toLocaleString(undefined, { minimumIntegerDigits: 2, useGrouping: false }) + '' +
            newDob.getDate().toLocaleString(undefined, { minimumIntegerDigits: 2, useGrouping: false })
    ...
    if (!(parseInt(idNumber.charAt(10)) === 0 || parseInt(idNumber.charAt(10)) === 1)) { return false }

    // Final Number Check
    const numberArray = idNumber.split('').map(Number)
    const oddSum = numberArray[0] + numberArray[2] + numberArray[4] + numberArray[6] + numberArray[8] + numberArray[10]
    const evenField = parseInt(idNumber[1] + idNumber[3] + idNumber[5] + idNumber[7] + idNumber[9] + idNumber[11]) * 2
    const evenFieldSum = evenField.toString().split('').map(Number).reduce((a, b) => a + b, 0)
    const fullSum = oddSum + evenFieldSum

    const year = (numberArray[0].toString() + numberArray[1].toString()).toLocaleString(undefined,
        { minimumIntegerDigits: 2, useGrouping: false })
    const isYearValid = year === "00" || parseInt(year) >= 1 && parseInt(year) <= 99
    const month = (numberArray[2].toString() + numberArray[3].toString()).toLocaleString(undefined,
        { minimumIntegerDigits: 2, useGrouping: false })
    const isMonthValid = parseInt(month) >= 1 && parseInt(month) <= 12
    const day = (numberArray[4].toString() + numberArray[5].toString()).toLocaleString(undefined,
        { minimumIntegerDigits: 2, useGrouping: false })
...
export const validateIncomeTaxNumber = (incomeTaxNumber) => {
    if (!incomeTaxNumber || incomeTaxNumber.length !== 10 || isNaN(incomeTaxNumber)) { return false }
...
export const validateGenderOfId = (idNumber, genderType) => {
    ...
    else if (genderValue > 4999 && genderValue < 10000 && genderType === 'Female') {
        return false
    }
...
export const validatePercentageTotal = percentages => {
    return getTotalPercentage(percentages) === 100 && validateNoZeroPercentage(percentages)
}
